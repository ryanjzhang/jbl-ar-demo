...
THREE.FlexMaterial = function (spec) {
    ...
    //same handy function
    function mix(a, b, t) {
        a.set(b.x * t + a.x * (1 - t), b.y * t + a.y * (1 - t), b.z * t + a.z * (1 - t));
    }
    ...
    vertexShaderSource = tweak_shaderAdd(vertexShaderSource, '#include <common>', 'uniform mat4 modelMatrixDelayed;\n' + 'uniform sampler2D flexMap;\n'
    //+'uniform float flexRatio;\n' //MT223
    );
    vertexShaderSource = tweak_shaderDel(vertexShaderSource, '#include <worldpos_vertex>');
    vertexShaderSource = tweak_shaderRepl(vertexShaderSource, '#include <project_vertex>', "vec4 worldPosition = modelMatrix * vec4( transformed, 1.0 );\n\
    ...
    var uniforms = _extends({}, phongShader.uniforms, uniforms0);
